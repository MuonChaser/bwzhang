/* Sidebar toggle: hide/show sidebar and persist state in localStorage */
(function () {
    // No localStorage: automatic behavior (observer/scroll) has highest priority.
    ...
    function init() {
        ...
        // Click toggles (manual), automatic observer and scroll may still override
        ...
        if (autoEnabled) {
            // IntersectionObserver: automatic hide/show has highest priority
            // use a slightly higher visible threshold so small partly-visible sidebar
            // counts as not visible (more sensitive to hiding)
            ...
            window.addEventListener('scroll', function () {
                ...
                var delta = y - lastY;
                // if user scrolls up more than 10px, reveal sidebar (more sensitive)
                if (delta < -10) {
                    setCollapsed(false, btn);
                }
                lastY = y;
